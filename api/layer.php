...
if ($process == "b2b_calc_shippingFee") {
    ...
    if (!isset($_GET["b2b_pro_main_code"])) {
        $json = json_encode(["success" => false, "validation_msg" => "'b2b_pro_main_code' parameter was not provided"]);
        echo $json;        
        exit();
    }

    if (!isset($_GET["b2b_pro_variant_code"])) {
        $json = json_encode(["success" => false, "validation_msg" => "'b2b_pro_variant_code' parameter was not provided"]);
        echo $json;        
        exit();
    }
    //
    $b2b_pro_main_code = $_GET["b2b_pro_main_code"];
    $b2b_pro_variant_code = $_GET["b2b_pro_variant_code"];
    //    
    $request_url = $zc_url . "/sync_product?publickey=n4uvny5MsTmrrPQ4PUDdkdRKR&b2b_pro_main_code=" . $b2b_pro_main_code . "&b2b_pro_variant_code=" . $b2b_pro_variant_code;
    $response = invokeURL($request_url, [], "GET",  []);
}
